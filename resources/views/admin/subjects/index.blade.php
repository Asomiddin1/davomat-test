<x-layouts.admin>
     <div class="flex-1 p-6 md:p-8">
        ...
        <div class="bg-white p-4 rounded shadow mb-6">
            <form method="POST" action="{{ route('subjects.store') }}" class="flex gap-4">
                @csrf
                <input 
                    type="text" 
                    name="name" 
                ...
                <tbody>
                    @forelse($subjects as $subject)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="border p-3">{{ $subject->name }}</td>
                            <td class="border p-3 text-center">
                                <form method="POST" action="{{ route('subjects.destroy', $subject->id) }}" onsubmit="return confirm('O‘chirilsinmi?')">
                                    @csrf
                                    @method('DELETE')
